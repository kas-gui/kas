...
impl<A, T: SpinValue> SpinGuard<A, T> {
    fn new(range: RangeInclusive<T>, state_fn: Box<dyn Fn(&ConfigCx, &A) -> T + Send>) -> Self {
...
mod SpinBox {
    ...
    pub struct SpinBox<A, T: SpinValue> {
        ...
        on_change: Option<Box<dyn Fn(&mut EventCx, &A, T) + Send>>,
    }
    ...
    impl Self {
        ...
        pub fn new(
            ...
            state_fn: impl Fn(&ConfigCx, &A) -> T + Send + 'static,
        ) -> Self {
            SpinBox {
        ...
        #[inline]
        #[must_use]
        pub fn with_msg<M>(self, f: impl Fn(T) -> M + Send + 'static) -> Self
        where
        ...
        #[inline]
        #[must_use]
        pub fn with(mut self, f: impl Fn(&mut EventCx, &A, T) + Send + 'static) -> Self {
