...
use syn::{
    parse2, parse_quote, token::Eq, Expr, Ident, ImplItem, Index, ItemImpl, Member, Token, Type,
};
...
pub fn widget(mut args: WidgetArgs, scope: &mut Scope) -> Result<()> {
    ...
    if let Some(inner) = opt_derive {
        ...
        scope.generated.push(quote! {
            ...
            impl #impl_generics ::kas::WidgetNode for #impl_target {
        ...
        let handle_messages = quote! {
            #[inline]
            fn handle_messages(&mut self, mgr: &mut ::kas::event::EventCx<Self::Data>) {
                self.#inner.handle_messages(mgr);
            }
        };
...
pub fn impl_widget_children(
    ...
    impl_target: &Toks,
    core_path: &Toks,
    children: &Vec<(Member, Span, Option<Box<Expr>>)>,
    ...
    if count != 0 {
        ...
        // NOTE: must unpack #impl_target to use this
