...
impl TextButton<VoidMsg> {
    ...
    /// closure `f` is called. The message generated by `f`
    /// is returned for handling through the parent widget (or other ancestor).
    #[inline]
    pub fn on_push<M, F: Fn() -> M + 'static>(self, f: F) -> TextButton<M> {
        TextButton {
            core: self.core,
            keys1: self.keys1,
...
impl<M: 'static> TextButton<M> {
    ...
    /// is returned for handling through the parent widget (or other ancestor).
    #[inline]
    pub fn new_on<S: Into<AccelString>, F: Fn() -> M + 'static>(label: S, f: F) -> Self {
    ...
    where
        M: Clone,
    {
        Self::new_on(label, move || msg.clone())
    }
...
impl<M: 'static> event::Handler for TextButton<M> {
    ...
    fn handle(&mut self, _: &mut Manager, event: Event) -> Response<M> {
        match event {
            Event::Activate => self.on_push.as_ref().map(|f| f()).into(),
